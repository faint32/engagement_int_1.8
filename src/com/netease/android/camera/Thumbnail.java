...
public class Thumbnail {
	...
	public static Thumbnail getLastThumbnailFromFile(File filesDir,
	...
	public static int getLastThumbnailFromContentResolver(
		...
		if (Util.isUriValid(lastMedia.uri, resolver)) {
			result[0] = createThumbnail(lastMedia.uri, bitmap,
					lastMedia.orientation);
			return THUMBNAIL_FOUND;
		}
		return THUMBNAIL_DELETED;
	}

	public static int getLastVideoThumbnailFromContentResolver(
			ContentResolver resolver, Thumbnail[] result) {
		Media video = getLastVideoThumbnail(resolver);
		if (video == null)
			return THUMBNAIL_NOT_FOUND;
		...
		// If there is only image or video, get its thumbnail. If both exist,
		// get the thumbnail of the one that is newer.
		Bitmap bitmap = Video.Thumbnails.getThumbnail(resolver, video.id,
				Video.Thumbnails.MINI_KIND, null);
		Media lastMedia = video;
