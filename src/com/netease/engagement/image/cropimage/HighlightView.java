...
class HighlightView {
	...
	public HighlightView(View ctx) {
        mContext = ctx;
        if(Build.VERSION.SDK_INT >= VERSION_CODES.HONEYCOMB){//4.0上默认开启硬件加速后不支持canvas.clipPath,这里禁掉
            ctx.setLayerType(View.LAYER_TYPE_SOFTWARE, null);
        }
    }

    private void init() {
        mResizeDrawableWidth = mContext.getResources().getDrawable(R.drawable.camera_crop_width);
        mResizeDrawableHeight = mContext.getResources().getDrawable(R.drawable.camera_crop_height);
        mResizeDrawableDiagonal = mContext.getResources().getDrawable(R.drawable.indicator_autocrop);
    }
