...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	/**是否允许HeaderLayout和FooterLayout允许可见性改变*/
	...
	public View customerCover;
	private ImageView customerRound;
	private ImageView customerArrow;
	private ImageView customerLoading;
	private final int customerPullMinDelta = 30;
	...
	//此处下拉刷新的实现方式是滚动ListView外层的容器，而不是ListView本身，这样拓展性更好
	@Override
	public final boolean onInterceptTouchEvent(MotionEvent event) {
		...
		switch (action) {
			case MotionEvent.ACTION_MOVE: {
			    ...
			    //正在刷新加载数据并且不允许在此时进行滚动
			    if(DEBUG){Log.d(LOG_TAG, "mScrollingWhileRefreshingEnabled: " + mScrollingWhileRefreshingEnabled);}
	...
	final void setState(State state, final boolean... params) {
		...
		switch (mState) {
			case RESET:
				onReset();
				if (customerArrow.getVisibility() == INVISIBLE) {
					customerArrow.setVisibility(VISIBLE);
					customerRound.setVisibility(VISIBLE);
					customerLoading.setVisibility(INVISIBLE);
					customerLoading.clearAnimation();
				}
				break;
	...
	private void customerOnRefreshing(boolean doScroll) {
		if (doScroll) {
				...
				switch (mCurrentMode) {
					...
					case PULL_FROM_START:
						...
						postDelayed(new Runnable() {
							@Override
							public void run() {
								customerArrow.setVisibility(INVISIBLE);
								customerRound.setVisibility(INVISIBLE);
								customerLoading.setVisibility(VISIBLE);
								RotateAnimation rotate = new RotateAnimation(0, 359, customerLoading.getWidth()/2, customerLoading.getHeight()/2);
								rotate.setDuration(1000);
								rotate.setRepeatCount(Animation.INFINITE);
								rotate.setInterpolator(new LinearInterpolator());
								customerLoading.startAnimation(rotate);
							}
						}, 50);
						break;
				}
	 ...
	 * 仅仅用作文字的处理，真正的UI动画等放在onRefreshing里面处理
	 */
	...
	protected final void refreshLoadingViewsSize() { 
		...
		//通过设置负数padding将header和footer进行隐藏
	...
	protected void onScrollChanged(int l, int t, int oldl, int oldt) {
		...
		int newScrollValueABS = Math.abs(t);
		if (newScrollValueABS > itemDimension) {
			RelativeLayout.LayoutParams lp = (RelativeLayout.LayoutParams) customerRound.getLayoutParams();
			lp.bottomMargin = newScrollValueABS - itemDimension;
			customerRound.setLayoutParams(lp);
		} else {
			RelativeLayout.LayoutParams lp = (RelativeLayout.LayoutParams) customerRound.getLayoutParams();
			lp.bottomMargin = 0;
			customerRound.setLayoutParams(lp);
		}
		if (newScrollValueABS > itemDimension + customerPullMinDelta) {
			customerArrow.setBackgroundResource(R.drawable.pull_arrow_up);
		} else {
			customerArrow.setBackgroundResource(R.drawable.pull_arrow_down);
		}
	}
	...
	protected void updateUIForMode() {
		...
		// If we're not using Mode.BOTH, set mCurrentMode to mMode, otherwise set it to pull down
		mCurrentMode = (mMode != Mode.BOTH) ? mMode : Mode.PULL_FROM_START;
	}

	/**将refreshableView添加到FrameLayout中，再将FrameLayout作为LinearLayout的child*/
	private void addRefreshableView(Context context, T refreshableView) {
		mRefreshableViewWrapper = new FrameLayout(context);
		mRefreshableViewWrapper.addView(refreshableView, ViewGroup.LayoutParams.MATCH_PARENT,
				ViewGroup.LayoutParams.MATCH_PARENT);

		//将ex.ListView添加到最后，index为-1代表添加到最后
	...
	private void init(Context context, AttributeSet attrs) {
		...
		customerCover = LayoutInflater.from(context).inflate(R.layout.pull_to_refresh_header_vertical_cover, null);
		customerRound = (ImageView) customerCover.findViewById(R.id.customer_round);
		customerArrow = (ImageView) customerCover.findViewById(R.id.customer_arrow);
		customerLoading = (ImageView) customerCover.findViewById(R.id.customer_loading);
		mHeaderLayout.addView(customerCover);
	...
	private void pullEvent() {
		...
		if (newScrollValue != 0 && !isRefreshing()) {
			...
			if (mState != State.PULL_TO_REFRESH && itemDimension + customerPullMinDelta >= Math.abs(newScrollValue)) {
				setState(State.PULL_TO_REFRESH);
			} else if (mState == State.PULL_TO_REFRESH && itemDimension + customerPullMinDelta < Math.abs(newScrollValue)) {
