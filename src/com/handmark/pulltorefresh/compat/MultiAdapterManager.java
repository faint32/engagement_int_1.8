...
public class MultiAdapterManager {

    private PullToRefreshListView mRrefreshList;
    ...
    public MultiAdapterManager(PullToRefreshListView refreshList) {
        mRrefreshList = refreshList;
        mAdapterList = new ArrayList<BaseAdapter>();
        mStatusList = new ArrayList<MultiAdapterManager.ListViewStatus>();
    }
    ...
    public void addAdapter(BaseAdapter adapter, String noContentStr) {
        mAdapterList.add(adapter);
    ...
    public void setCurrentAdapter(int index) {
        ...
        if (mRrefreshList.getRefreshableView().getFirstVisiblePosition() < mCommonHeaderCount
            ...
            mRrefreshList.setAdapter(mAdapterList.get(mCurrentAdapterIndex));
    ...
    private class ListViewStatus {
        ...
        public void saveStatus(PullToRefreshListView refreshList) {
            loadingStatus = refreshList.getLoadingState();
            hasMore = refreshList.hasMore();
        }

        public void loadStatus(PullToRefreshListView refreshList) {
            refreshList.setHasMore(hasMore);
            if (refreshList instanceof LoadingListView) {
                resetLoadingString((LoadingListView)refreshList);
            }
         ...
         * 重置LoadingListView文案
         */
        public void resetLoadingString(LoadingListView loadingList) {
